...
class BaseResend:
    ...
    async def send_mail(self, recipients: Union[EmailStr, List[EmailStr]], subject: str, content: str):
        ...
        params = {"from": f"{self.sender_name} <{self.sender_email}>", "to": recipients_list, "subject": subject, "html": content}
        try:
...
class UserMailService(BaseResend):
    ...
    async def send_verification_email(self, recipient: EmailStr, fullname: str, verify_token: str) -> dict:
        data = {}
        data["fullname"] = fullname
        data["verification_link"] = settings.verification_link + verify_token
